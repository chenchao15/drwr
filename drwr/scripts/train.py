...
from util.train import get_trainable_variables, get_learning_rate, get_path
from util.losses import regularization_loss
from util.fs import mkdir_if_missing
from util.data import tf_record_compression
from scripts.predict_eval import test_one_step
...
def parse_tf_records(cfg, serialized):
    ...
    features = {
        ...
        'mask_sdf': tf.FixedLenFeature([num_views, image_size, image_size, 1],tf.float32),
    }
...
def train():
    ...
    with summary_writer.as_default(), tfsum.record_summaries_every_n_global_steps(10):
        ...
        model = model_base.ModelPointCloud(cfg, global_step)
        ...
        # loss
        task_loss, c_loss, s_loss, p_loss, cd_loss,w= model.get_loss(inputs, outputs)
    ...
    with sess, summary_writer.as_default():
        ...
        while global_step_val < cfg.max_number_of_steps:
            ...
            dt = t1 - t0
            print(f"step: {global_step_val},loss = {loss_val:.8f} ({dt:.3f} sec/step)")
                
            if global_step_val % 50000 == 0:
